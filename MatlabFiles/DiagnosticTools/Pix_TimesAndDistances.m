...
dispAlt = sprintf('Max Alt:\n   %d m\n   %d ft\n',round(max(GPS(:,10))-min(GPS(:,10)),0),round(unitsratio('feet','meters')*(max(GPS(:,10))-min(GPS(:,10))),0));
dispDist = sprintf('Max Distance:\n   %d m\n   %d ft\n',round(max(d),0),round(unitsratio('feet','meters')*max(d),0));
dispTO = sprintf('Takeoff Time (%s):\n   %s\n',GPS_stw.TimeZone,GPS_stw(1));
dispLND = sprintf('Land Time (%s):\n   %s\n',GPS_stw.TimeZone,GPS_stw(2));
