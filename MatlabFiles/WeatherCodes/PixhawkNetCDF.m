...
lblTeensy = uilabel(tabDP,'Text','5HP Suites','Position',...
    [550,700,150,30],'FontSize',24);
spnTeensy = uispinner(tabDP,'Limits',[0 10],'Value',0,'Position',...
    [550,650,125,30],'UserData','Teensy Suite');
lblAnem = uilabel(tabDP,'Text','Anemometers','Position',...
    [700,700,150,30],'FontSize',24);
spnAnem = uispinner(tabDP,'Limits',[0 10],'Value',0,'Position',...
    [700,650,125,30],'UserData','Anemometer');
...
GObtn = uibutton(tabDP,'push','Text','Process','Position',[600 550 100 30],...
    'ButtonPushedFcn',@(GObtn,event) startProcessing(fig, tabgp, iMetspn, spnTeensy, spnAnem));
...
function startProcessing(fig, tabgp, iMetspn, spnTeensy, spnAnem)
...
% Parse Anemometer data (if enabled in UI)
for i=1:length(spnTeensy)
    spnData = spnAnem(1,i);
    % Confirm spinner has Teensy data before parsing
    if(strcmpi(spnData.UserData,'Anemometer'));
        for j=1:spnData.Value
            processAnem(baseNameNoExt);
        end
    end
end
...
answer = questdlg('Save data as netCDF file?', ...
    'netCDF File Output Format', ...
    'Yes','No','Yes');
...
% INPUT
% * baseNameNoExt - Pix base file name with no extension for filepath details
...
%% FUNC: processAnem - Read Anemometer data, determine which to parse as
% Determine which of Anemometer type to parse
% INPUT
% * baseNameNoExt - Pix base file name with no extension for filepath details
function processAnem(baseNameNoExt)
global AnemNumber
global parsedAnem

% Get the name of the file that the user wants to use.
[baseNameNoExtAnem, ~, folderAnem, fullInputMatFileNameAnem]...
    = file2open('*.csv','Select an Anemometer .CSV file');
% Read data in as matrix and reject first row
dataRaw = readmatrix(fullInputMatFileNameAnem);
data = dataRaw(2:end,:);

% Remove any data entries that did not print properly 
% * Excluding Pixhawk data
...
% Check type of Anemometer by checking specific columns
% Y92 has barometric pressure in column 6 (~980 mbar usual)
% Y81 has temperature in column 6 (~25 C usual)
if(max(data(:,6))>300)
    % If data exists, iterate MHP number (in case more than one is used)
    AnemNumber = AnemNumber+1;
    outputAnem = parseY92(data, folderAnem, baseNameNoExt);
else
    AnemNumber = AnemNumber+1;
    outputAnem = parseY81(data, folderAnem, baseNameNoExt);
end


% Save data according to Teensy call number
% Preserves all Teensy-specific data even if some entries are empty
parsedAnem{AnemNumber,1} = baseNameNoExtAnem;
parsedAnem{AnemNumber,2} = outputAnem;
parsedAnem{AnemNumber,3} = outputAnem.Properties.VariableNames;
...
%% FUNC: parseY81 - Parse and process Young 81000 data
% Interpolate to backfill emtpy Pixhawk timing data
% Output external table (DateUTC, TimeUTC) to user workspace
% Preseve internal table (DatenumUTC) for other uses
% INPUT
% * data - Full Anemometer dataset
% * folder - Folder containing raw Anemometer data
% * baseNameNoExt - Pix base file name with no extension or filepath details
% OUTPUT
% * outputY81 - Internal version of resulting table (DatenumUTC)
function outputY81 = parseY81(data,folder,baseNameNoExt)

global parseRangeUTC
global AnemNumber

% Define timing variables for interpolation
boardTime = data(:,1);
unixTime = data(:,7);
pixTime = data(:,8);

% Backfill Pixhawk BOOT and UNIX times via linear interpolation
% Check if valid Pixhawk board time exists, otherwise error out
if(max(pixTime) > 0)
    % Interpolate between Pixhawk board and Teensy board times to backfill
    %     empty entries
    filledPixT(:,1)=timeInterpolation(boardTime(:,1),pixTime(:,1));
    % Check if valid Unix time was sent by Pixhawk, otherwise error out
    if(max(unixTime)>0)
        % Interpolate between Pixhawk board and Teensy board times to   
        %     backfill empty entries
        filledUnixT(:,1)=timeInterpolation(boardTime(:,1),unixTime(:,1));
    else
        error('Pixhawk GPS data not in file. Unix time cannot be parsed.');
    end
else
   error('Pixhawk data not sent. Times cannot be parsed.'); 
end
...
Y81_tableExternal = table(boardTime(:,1),filledPixT(:,1),anemTimeOut,...
    data(:,2),data(:,3),data(:,4),data(:,5),data(:,6),...
    'VariableNames', {'Board Time from PowerUp (msec)',...
    'Pix Time from PowerUp (msec)','Time from parse (sec)',...
    'Wind Speed (m/s)','Wind Directoin (deg)','Wind Elevation (deg)',...
    'Speed of Sound (m/s)','Temperature (°C)'} );
Y81_tableExternal.Properties.Description = sprintf('Y81Anem%d',Y81AnemNumber);
% Output table to user workspace
assignin('base',sprintf('Y81Anem_table%d',Y81AnemNumber),Y81_tableExternal);
% Save tabele for user review
table2saveCSV(baseNameNoExt, folder, Y81_tableExternal)

% Generate internal-use table that uses DatenumUTC
% ADD DATENUME AS ENTRY
Y81_tableInternal = table(boardTime(:,1),filledPixT(:,1),anemTimeOut,...
    data(:,2),data(:,3),data(:,4),data(:,5),data(:,6),...
    'VariableNames', {'Board Time from PowerUp (msec)',...
    'Pix Time from PowerUp (msec)','Time from parse (sec)',...
    'Wind Speed (m/s)','Wind Directoin (deg)','Wind Elevation (deg)',...
    'Speed of Sound (m/s)','Temperature (°C)'} );
Y81_tableInternal.Properties.Description = sprintf('Y81Aenm%d',Y81AnemNumber);

% Ouput internally-used table
outputY81 = Y81_tableInternal;
end
%% FUNC: parseY92 - Parse and process Young 92000 data
% Interpolate to backfill emtpy Pixhawk timing data
% Output external table (DateUTC, TimeUTC) to user workspace
% Preseve internal table (DatenumUTC) for other uses
% INPUT
...
% * baseNameNoExt - Pix base file name with no extension or filepath details
% OUTPUT
% * outputY92 - Internal version of resulting table (DatenumUTC)
function outputY92 = parseY92(data,folder,baseNameNoExt)
...
%% FUNC: processTS - Parse and process Trisonica data
...
if(lastExtern ~= length(externTime))
...
outputArray = [beginning; interpolatedArray(firstExtern:lastExtern,1); ending];
