...
%DFL_NewOld = 'Old';           % Can be deleted
graphToggle = 'Yes';            % Show plots of parsed data
animateToggle = 'Yes';         % Flight animation based on GPS and Alt(AGL)
%ArduPilotType = 'Quad-Plane';  % Quad-Plane, Fixed Wing, Quadcopter (can be deleted)
StateSpace = 'Yes';             % Pixhawk recorded SS variable output (new file)
...
GPS_out = 'Yes';               % Output GPS data as individual file (lat, long, alt)
Sensor_out = 'Yes';             % Output parsed sensor data (iMet seperate from 5HP/TPH)
Attitude_out = 'Yes';          % Output parsed attitude data
...
if (strcmpi(graphToggle,'Yes'))
    ...
    t_cube = table2array(CTUN_table(:,3));
    ...
    STAT_mat = table2array(STAT_table);
    ...
    if (strcmpi(Overlay,'Yes'))
        ...
        if(strcmpi(MHPValue,'Yes'))
            if(exist('MHP_table'))
                % Groundspeed plot
                plt1 = subplot(5,1,1);
                plot(t_GPS,GPS_mat(:,8),'k',t_cube,CTUN_mat(:,4),'b',t_NKF,NKF2_mat(:,6),'r',MHP_mat(:,2)/1000,MHP_mat(:,4),'g')
            ...
            % Groundspeed plot
            plt1 = subplot(5,1,1);
            plot(t_GPS,GPS_mat(:,8),'k',t_cube,CTUN_mat(:,4),'b',t_NKF,NKF2_mat(:,6),'r')
        ...
        % TIV uses -20 to 50 to see high AoA landing
        plt3 = subplot(5,1,3);
        plot(t_cube,ATT_mat(:,5),'b',t_cube,zero,'r:')
        title('Aircraft Pitch Angle vs Time')
        ylabel({'Aircraft Pitch';'Angle (°)'})
        %xticks([150:20:370])
        ylim([-20 50])
        ...
        if(strcmpi(TPHValue,'Yes') && strcmpi(MHPValue,'Yes') && strcmpi(iMetValue,'Yes'))
            if(exist('TPH_mat') && exist('MHP_table') && exist('iMet_table'))
                ...
                plt1 = subplot(3,1,1);
                plot(t_cube,CTUN_mat(:,4),'k',MHP_mat(:,3),MHP_mat(:,4),'r',MHP_mat(:,3),MHP_mat(:,5),'b',MHP_mat(:,3),MHP_mat(:,6),'g');
        ...
        % Groundspeed plot
        plt1 = subplot(4,1,1);
        plot(t_GPS,GPS_mat(:,8),'b-',t_cube,CTUN_mat(:,4),'r-',t_NKF,NKF2_mat(:,6),'g-')
    ...
    if (strcmpi(indvToggle,'Yes'))
        ...
        while true
            ...
            y_n2(n)=CTUN_mat(find(t_cube>=x_n(n),1),4);     % Airspeed
            y_n3(n)=NKF2_mat(find(t_NKF>=x_n(n),1),6);     % Windspeed
            y_n4(n)=RCOU_mat(find(t_cube>=x_n(n),1),6);     % Throttle Percent
            y_n5(n)=ATT_mat(find(t_cube>=x_n(n),1),5); % Aircraft Pitch
            y_n6(n)=GPS_mat(find(t_GPS>=x_n(n),1),7);    % Altitude
            y_n7(n)=RCOU_mat(find(t_cube>=x_n(n),1),4);  % Pitch PWM
            
            % Groundspeed plot
            subplot(4,1,1);
            plot(t_GPS,GPS_mat(:,8),'b',t_cube,CTUN_mat(:,4),'r',t_NKF,NKF2_mat(:,6),'g',x_n,y_n1,'kx',x_n,y_n2,'kx',x_n,y_n3,'kx')
            title('Groundspeed, Airspeed, and Windspeed vs Time')
            ylabel({'Groundspeed (blue)';'Airspeed (red)';'Windspeed (green)';'(m/s)'})
            xlim([min(t_GPS) max(t_GPS)])
            ...
            % TIV uses -20 to 50 to see high AoA landing
            subplot(4,1,3);
            plot(t_cube,ATT_mat(:,5),'b',t_cube,zero,'r:',x_n,y_n5,'kx');
            title('Aircraft Pitch Angle vs Time');
            ylabel({'Aircraft Pitch';'Angle (°)'});
            xlim([min(t_cube) max(t_cube)]);
            ylim([-20 50]);
...
if(strcmpi(animateToggle, 'Yes'))
    id = ones(length(GPS_mat(:,6)), 1);
    t_plot=(1:length(GPS_mat(:,4)));
    obj1 = [GPS_mat(:,4), GPS_mat(:,5), GPS_mat(:,7), t_plot', id];
